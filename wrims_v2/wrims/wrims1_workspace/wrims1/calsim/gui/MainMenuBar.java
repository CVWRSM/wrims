...
public class MainMenuBar {
	...
	public static int[] projectSubKeys = { KeyEvent.VK_N, KeyEvent.VK_O,
   ...
   */
	...
	JMenu createProjectSubmenu() {
		...
		newitem.setAccelerator(KeyStroke.getKeyStroke(projectSubKeys[0],
				KeyEvent.CTRL_MASK));
		openitem.setAccelerator(KeyStroke.getKeyStroke(projectSubKeys[1],
				KeyEvent.CTRL_MASK));
		loaditem.setAccelerator(KeyStroke.getKeyStroke(projectSubKeys[2],
				KeyEvent.CTRL_MASK));
		saveitem.setAccelerator(KeyStroke.getKeyStroke(projectSubKeys[3],
				KeyEvent.CTRL_MASK));
		saveAsItem.setAccelerator(KeyStroke.getKeyStroke(projectSubKeys[4],
				KeyEvent.CTRL_MASK));
	...
	JMenu createOpenBaseMenu() {
		...
		dvaritem.setAccelerator(KeyStroke.getKeyStroke(openBaseSubKeys[0],
				KeyEvent.CTRL_MASK));
		svaritem.setAccelerator(KeyStroke.getKeyStroke(openBaseSubKeys[1],
				KeyEvent.CTRL_MASK));
	...
	JMenu createUnitsMenu() {
		 ...
		 * _oldUnits.setSelected(AppUtils.useStoredUnits); keepUnits(); }else{
		 */
		if (AppUtils.useCFS)
			cfs.doClick();
		else
    ...
    */
	JMenu createMonthlyTableTypeMenu() {
		...
		/*
		 * JMenuItem formatItem = new JMenuItem("Edit Fraction Digits");
		 * formatItem.addActionListener( new ActionListener(){ public void
		 * actionPerformed(ActionEvent evt){ String str = (String)
		 * JOptionPane.showInputDialog(GuiUtils.getMainPanel(),
		 * "Input Number of Fraction Digits for MT Format", "Enter Integer",
		 * JOptionPane.PLAIN_MESSAGE, null, null, "0"); if (str==null) return;
		 * int fracDigits = new Integer(str).intValue();
		 * ReportUtils.setFormat(fracDigits,1,fracDigits,false); } });
		 * 
		 * // Format item for monthly table column width JMenuItem formatItem2 =
		 * new JMenuItem("Edit Column Width"); formatItem2.addActionListener(
		 * new ActionListener(){ public void actionPerformed(ActionEvent evt){
		 * String str = (String)
		 * JOptionPane.showInputDialog(GuiUtils.getMainPanel(),
		 * "Input Column Width for Monthly Table Format", "Enter Integer",
		 * JOptionPane.PLAIN_MESSAGE, null, null, "8"); if (str==null) return;
		 * int colwidth = new Integer(str).intValue();
		 * ReportUtils.setSpaceCount(colwidth); } });
		 */
